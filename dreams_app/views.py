...
class DreamCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if image_bytes:
            ...
            messages.warning(
                self.request,
                "Image couldn’t be generated for this dream (it may contain sensitive content). "
                "Try rephrasing it with less graphic/sexual detail."
            )
...
class DreamUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        # Regenerate if text changed OR missing analysis/image
        ...
        if needs_regen:
            ...
            if not _consume_ai_analysis(self.request.user):
                ...
                # If text changed, clear old image (no longer matches)
            ...
            if image_bytes:
                ...
                messages.warning(
                    self.request,
                    "Image couldn’t be regenerated for this dream (it may contain sensitive content). "
                    "Try rephrasing it with less graphic/sexual detail."
                )
                # keep existing image if text didn't change, otherwise remove it
                if "text" in form.changed_data and dream.image:
                    dream.image.delete(save=False)
                    dream.image = None
...
def toggle_favorite(request, pk):
    ...
    favorite, created = Favorite.objects.get_or_create(user=request.user, dream=dream)
